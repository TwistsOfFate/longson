module hazard(input  logic [4:0] d_rs, d_rt, e_rs, e_rt,
              ...
              output logic [1:0] d_forwarda, d_forwardb,
...
always@(*)
    begin
        d_forwarda = 2'b00 ; d_forwardb = 2'b00 ;
        if(d_rs != 0)
            if((d_rs == m_writereg) && m_regwrite) d_forwarda = 2'b10 ;
            else if((d_rs == w_writereg) && w_regwrite) d_forwarda = 2'b01 ;
            else d_forwarda = 2'b00 ;
        if(d_rt != 0)
            if((d_rt == m_writereg) && m_regwrite) d_forwardb = 2'b10 ;
            else if((d_rt == w_writereg) && w_regwrite) d_forwardb = 2'b01 ;
            else d_forwardb = 2'b00 ; 
